...
- time_candidates: for each concrete date the renter proposes in the NEWEST message, date as YYYY-MM-DD (resolve "tomorrow"/"Friday" against Phoenix now, never a past date; a bare weekday name ALWAYS means the NEXT occurrence of that weekday, 1 to 7 days ahead - "Monday" said on a Sunday is tomorrow, never a week later), time as 24h HH:MM if they named an exact time else null, after/before as 24h HH:MM bounds ("after 3" -> after 15:00), raw = their words.
...
def _json_call(prompt: str, schema: dict, max_tokens: int,
               what: str) -> dict | None:
    """The one Haiku call site. Both micro-calls in this module want the same
    thing - a short, schema-constrained JSON answer with a hard 10s budget and
    a single retry - and both must return None rather than raise, because each
    caller has its own conservative fallback. Returns None when the key is
    missing or anything at all goes wrong."""
...
def _api_call(renter_text: str, transcript: str, now_phx: datetime) -> dict | None:
    return _json_call(
        ...
        SCHEMA, 1536, "Haiku classify (falling back to regex)")
...
def review_reply(transcript: str, outgoing_body: str, template: str) -> dict:
    ...
    # Distinguish "no key configured" from "the call blew up": both send, but
    # the reason lands in the audit trail and they mean different things.
    ...
    out = _json_call(
        ...
        REVIEW_SCHEMA, 256, "review_reply (failing open)")
    if out is None:
