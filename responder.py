...
def acquire_send_lock(thread_id: str, stage_key: str, meta: dict,
    ...
    against Gmail. Returns None when we own the lock and the caller may send;
    otherwise the 'skipped:<why>' string the caller should return as-is.

    One copy of a preamble that used to be written out four times (send_stage
    plus the three booking paths) and had drifted between them: the booking
    copies stamped recovered="backfilled" and returned "skipped:recovered"
    for the situation send_stage called "backfilled-from-gmail" /
    "skipped:recovered-already-sent". Same event, one name now.
    """
...
def apply_labels(thread_id: str, stage_key: str, add: list, remove: list,
                 context: str = ""):
    """Apply Gmail labels, parking them for the cron retry if Gmail refuses.

    A label failure NEVER triggers a resend (that is the whole point of the
    send lock), so this logs and queues, and never raises. Replaces four
    copies of the same try/except that differed only in log wording.
    """
...
def send_stage(thread_id: str, stage_key: str, relay: str, body: str,
    ...
    blocked = acquire_send_lock(thread_id, stage_key, meta, trigger_message_id)
    if blocked:
        return blocked
...
def book_accepted_offer(thread_id, doc, first_name, address, relay, message_id) -> str:
    ...
    blocked = acquire_send_lock(thread_id, stage, {"template": "booking_fold"},
                                message_id)
    if blocked:
        return blocked
...
def book_proposed_time(thread_id, doc, first_name, address, relay, cls,
    ...
    for cand in cls.get("time_candidates", []):
        ...
        stage_probe = f"reply__{message_id}"
        blocked = acquire_send_lock(thread_id, stage_probe,
