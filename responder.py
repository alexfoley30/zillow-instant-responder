...
def _book_and_confirm(thread_id, stage_key, relay, body, template, message_id,
                      calendar_op, poke_msg, **transition_fields) -> str:
    """The ONLY function that books a showing and confirms it. Sibling of
    send_stage for the one case send_stage cannot cover: a send that must be
    preceded by a calendar write.

    THE ATOMIC BOOKING TRIPWIRE, in one place. Order is fixed and load-bearing:
    reserve -> review -> calendar -> send -> mark_sent -> labels -> transition
    -> FYI ping. The calendar write happens FIRST so a crash can only ever
    leave a booked event with no email (recoverable by hand) and never an
    email promising a slot nobody holds.

    `calendar_op` is a zero-arg callable that creates or folds the event and
    returns its id. Returns 'sent' | 'shadowed'-style 'skipped:<why>' strings,
    matching send_stage's contract. Callers own dry_run shadowing: the shadow
    payloads differ per path and none of this runs in a dry run.
    """
    ...
    try:
        ...
        # The event EXISTS and the renter was never told. This distinct
        # outcome is what tells a human to go look (before 2026-09-17 the
        # accepted-offer fold collapsed this into a generic book-failed).
    ...
    try:
        gm.modify_labels(thread_id, [HANDLED_LABEL], [AWAITING_LABEL])
    except Exception as e:  # noqa: BLE001 - label failure NEVER unbooks
    ...
    ledger.transition(thread_id, ledger.BOOKED, event_id=event_id,
                      **transition_fields)
    try:
        gm.poke_ping(poke_msg)
    except Exception as e:  # noqa: BLE001
        log.error("%s FYI ping failed: %s", template, e)
    return "sent"
...
def book_accepted_offer(thread_id, doc, first_name, address, relay, message_id) -> str:
    ...
    return _book_and_confirm(
        thread_id, stage, relay,
        T.booking_confirmation(first_name, address,
                               existing["when_human"], agent_name),
        "booking_fold", message_id,
        lambda: cal.fold_renter_into_event(existing["event"], first_name),
        f"Booked (added to existing showing): {first_name} - "
...
def book_proposed_time(thread_id, doc, first_name, address, relay, cls,
    ...
    for cand in cls.get("time_candidates", []):
        ...
        return _book_and_confirm(
            thread_id, f"reply__{message_id}", relay,
            T.booking_confirmation(first_name, address, when_human,
                                   agent["name"]),
            "booking_new", message_id,
            lambda: cal.create_showing_event(address, first_name, start_az, agent),
            jace_ping or (
