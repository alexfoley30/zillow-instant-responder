...
def _acquire_send_lock(thread_id: str, stage_key: str, meta: dict,
                       ...
                       recovered_note: str = "backfilled",
                       recovered_result: str = "skipped:recovered") -> str | None:
    """Win the Firestore create-once send lock for (thread, stage).

    Returns None when we own the lock and the caller may proceed; otherwise
    the result string the caller must return unchanged. A stale reserved/
    failed doc is resolved against the source of truth (the Gmail thread)
    before we take it over. Every renter-facing write goes through here -
    no lock, no send, ever.
    """
    ...
    if verdict == "recover":
        ...
        if gm.alex_replied_after(msgs, trigger_message_id):
            ledger.mark_sent(thread_id, stage_key, recovered=recovered_note)
...
def _review_or_fail(thread_id: str, stage_key: str, body: str,
                    template: str) -> str | None:
    """Run the pre-send review gate plus the failure handling every send site
    carries: mark the send doc failed and escalate to a human. Returns None to
    proceed, else the result string the caller must return."""
...
def _apply_labels(thread_id: str, stage_key: str, add: list, remove: list,
                  context: str = "send"):
    """Apply Gmail labels after a successful send. A label failure NEVER
    triggers a resend - it queues on the send doc for the cron retry."""
    if not add and not remove:
        return
    try:
        gm.modify_labels(thread_id, add, remove)
    except Exception as e:  # noqa: BLE001
        log.error("label failed %s %s after %s (queued for retry): %s",
...
def send_stage(thread_id: str, stage_key: str, relay: str, body: str,
    ...
    locked = _acquire_send_lock(
        thread_id, stage_key, meta, trigger_message_id,
        recovered_note="backfilled-from-gmail",
        recovered_result="skipped:recovered-already-sent")
    if locked is not None:
        return locked

    blocked = _review_or_fail(thread_id, stage_key, body,
...
def book_accepted_offer(thread_id, doc, first_name, address, relay, message_id) -> str:
    ...
    locked = _acquire_send_lock(thread_id, stage, {"template": "booking_fold"},
                                message_id)
    if locked is not None:
        return locked
    ...
    blocked = _review_or_fail(thread_id, stage, body, "booking_fold")
    if blocked is not None:
        return blocked
    try:
    ...
    _apply_labels(thread_id, stage, [HANDLED_LABEL], [AWAITING_LABEL], "fold")
...
def book_proposed_time(thread_id, doc, first_name, address, relay, cls,
    ...
    for cand in cls.get("time_candidates", []):
        ...
        if not (doc or {}).get("snap_offered") and not dry_run():
            ...
            if near:
                ...
                if diff_s <= 300:
                    ...
                    blocked = _review_or_fail(thread_id, stage_probe, body,
                                              "booking_fold_snap")
                    if blocked is not None:
                        return blocked
                    try:
        ...
        stage_probe = f"reply__{message_id}"
        locked = _acquire_send_lock(thread_id, stage_probe,
        ...
        body = T.booking_confirmation(first_name, address, when_human,
                                      agent["name"])
        blocked = _review_or_fail(thread_id, stage_probe, body, "booking_new")
        if blocked is not None:
            return blocked
        try:
