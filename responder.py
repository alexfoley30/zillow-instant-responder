...
def _claim_send(thread_id: str, stage_key: str, meta: dict,
                trigger_message_id: str = "") -> str | None:
    """Win the Firestore send lock for one stage, or say why we must not send.
    Returns None when the caller owns the lock and may proceed, otherwise the
    'skipped:<why>' string for the caller to return.

    THE ATOMIC BOOKING TRIPWIRE lives here. Every renter-facing path claimed
    the lock with its own copy of this dance (send_stage, the two booking
    funcs, the adjacency snap-fold) and they had drifted on the recovered
    marker text - one place now, so a fix to the reserve/recover/takeover
    order cannot land in three of four callers."""
    verdict = ledger.reserve_send(thread_id, stage_key, meta)
    if verdict in ("already-sent", "in-flight"):
        log.info("send claim skip (%s): %s %s", verdict, thread_id, stage_key)
...
def _gate_body(thread_id: str, stage_key: str, body: str,
               template: str) -> str | None:
    """Pre-send review plus the escalation a block owes Alex. Returns None to
    proceed, or the 'no-send:review-blocked' string for the caller to return."""
...
def _flush_labels(thread_id: str, stage_key: str, labels_add: list,
                  labels_remove: list, after: str):
    """Apply labels, queueing them for the cron retry if Gmail refuses. A
    label failure NEVER triggers a resend, so this never raises."""
    if not (labels_add or labels_remove):
        return
    try:
        ...
        log.error("label failed after %s %s %s (queued for retry): %s",
                  after, thread_id, stage_key, e)
...
def send_stage(thread_id: str, stage_key: str, relay: str, body: str,
    ...
    claim = _claim_send(thread_id, stage_key, meta, trigger_message_id)
    if claim:
        return claim

    blocked = _gate_body(thread_id, stage_key, body,
    ...
    _flush_labels(thread_id, stage_key, labels_add, labels_remove, "send")
...
def book_accepted_offer(thread_id, doc, first_name, address, relay, message_id) -> str:
    ...
    claim = _claim_send(thread_id, stage, {"template": "booking_fold"},
                        message_id)
    if claim:
        return claim
    ...
    blocked = _gate_body(thread_id, stage, body, "booking_fold")
    if blocked:
        return blocked
    try:
    ...
    _flush_labels(thread_id, stage, [HANDLED_LABEL], [AWAITING_LABEL], "fold")
    ...
    for cand in cls.get("time_candidates", []):
        ...
        if not (doc or {}).get("snap_offered") and not dry_run():
            ...
            if near:
                ...
                if diff_s <= 300:
                    claim = _claim_send(thread_id, stage_probe,
                    ...
                    blocked = _gate_body(thread_id, stage_probe, body,
                                         "booking_fold_snap")
                    if blocked:
                        return blocked
                    try:
                    ...
                    ledger.transition(thread_id, ledger.BOOKED, event_id=ev_id,
                                      snap_offered=True)
                    _flush_labels(thread_id, stage_probe, [HANDLED_LABEL],
        ...
        stage_probe = f"reply__{message_id}"
        claim = _claim_send(thread_id, stage_probe, {"template": "booking_new"},
                            message_id)
        if claim:
            return claim
        ...
        body = T.booking_confirmation(first_name, address, when_human,
                                      agent["name"])
        blocked = _gate_body(thread_id, stage_probe, body, "booking_new")
        if blocked:
            return blocked
        try:
        ...
        _flush_labels(thread_id, stage_probe, [HANDLED_LABEL], [AWAITING_LABEL],
                      "booking")
        ledger.transition(thread_id, ledger.BOOKED, event_id=event_id,
